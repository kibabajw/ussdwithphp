...
if (count($ussd_exploded_string) > 2 and count($ussd_exploded_string) <= 4) {
   // session_start();
    //if (!isset($_SESSION['user_id'])) {
        ...
        // ussd_stop($ussd_text);
        $sql = "SELECT * FROM user_sessions WHERE phoneNumber = ? && sessionid = ?";
    ...
    //} 
        ...
        $user_id_fromdb = $row['session_body'];
        switch ($ussd_exploded_string[2]) { # swich to check either is deposit or withdraw, check bal
            case 1:
                # deposit works...
                ...
                if (count($ussd_exploded_string) == 4) {
                    ...
                    $stmt = $conn->prepare($sql);
                    $stmt->execute([$user_id_fromdb, $deposit_amount]);
            ...
            case 2:
                ...
                if (count($ussd_exploded_string) == 4) {
                    ...
                    $stmt = $conn->prepare($sql);
                    $stmt->execute([$user_id_fromdb , $amount_withdrawable]);
...
function login($ussd_exploded_string, $conn, $phoneNumber, $sessionId){
    ...
    if (count($ussd_exploded_string) == 2) {
        ...
        if ($count == 0) {
            ...
            $sql = "INSERT INTO user_sessions (phoneNumber, sessionId, session_body) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$phoneNumber, $sessionId, $_SESSION['user_id']]);
